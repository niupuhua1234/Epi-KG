...
class MultivariateNormalNetwork(Posterior):
    ...
    #                         (parent_nodes_samples_normalized[..., j] - self.Z_lr[k][j])\
    #                         /(self.Z_up[k][j] - self.Z_lr[k][j])
    #                 X_node_k = self.X[..., self.active_indices[k]]
    #                 aux_shape = [sample_shape[0]] + [1] * X_node_k.ndim
    #                 X_node_k = X_node_k.unsqueeze(0).repeat(*aux_shape) # repeat to consistent with sample size 128
    ...
    #                                                 torch.einsum('abcd,a->abcd',
    #                                                              torch.sqrt(multivariate_normal_at_node_k.variance),
    #                                                              torch.flatten(base_samples[..., k])))[..., 0] #[...,0] squeze feature dim as each node is of feature dim ==1
    #                     elif my_aux.ndim == 5:
    #                         nodes_samples[...,k] = (multivariate_normal_at_node_k.mean +
    #                                                 torch.einsum('abcde,a->abcde',
    #                                                              torch.sqrt(multivariate_normal_at_node_k.variance),
    #                                                              torch.flatten(base_samples[..., k])))[..., 0]
    #                     else:
    #                         raise 'error'
    #                 else:
    #                     nodes_samples[..., k] = multivariate_normal_at_node_k.rsample()[0, ..., 0]# incase for root nodes should not arive there
    #                 nodes_samples_available[k] = True
    #                 t1 = time.time()
